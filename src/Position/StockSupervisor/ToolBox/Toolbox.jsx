...
const Toolbox = () => {
  ...
  const loadToolbox = async () => {
    try {
      const result = await axios.get(`${process.env.REACT_APP_API_URL}toolbox/gettoolbox`);
      ...
      const locationPromises = locationIds.map((id) =>
        axios.get(`${process.env.REACT_APP_API_URL}locations/${id}`)
      );
  ...
  const deleteTool = async (toolbox_id) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_URL}toolbox/delete/${toolbox_id}`
      );
      loadToolbox();
